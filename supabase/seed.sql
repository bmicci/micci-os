-- ============================================================
-- Micci OS — Financial Data Seed
-- Run this against your Supabase project to replace demo data
-- with real financial data.
--
-- Usage (Supabase CLI):
--   supabase db push --db-url $DATABASE_URL < supabase/seed.sql
...
INSERT INTO debt_accounts (name, account_type, balance, interest_rate, minimum_payment, recommendation, notes) VALUES
  ('SoFi Personal Loan',                'loan',        56958.92, 12.41,  1464.07, 'Roll into HELOC — 12.41% vs 6.85%',        NULL),
  ('LightStream Loan',                  'loan',        44018.42,  5.87,   577.47, 'Keep — 5.87% < HELOC rate',                 NULL),
  ('Chase Freedom Unlimited (7117)',    'credit_card', 22531.00,  0.00,   225.00, 'Promo 0% — pay from HELOC before expiry',   '0% promo BT'),
  ('Chase Freedom (9313)',              'credit_card', 13269.00,  0.00,   132.00, 'Promo 0% — pay from HELOC before expiry',   '0% promo BT'),
  ('Citi Diamond Preferred',            'credit_card', 12366.00,  0.00,   124.00, 'Promo 0% — pay from HELOC by Jun 2027',     '0% promo thru Jun 18 2027'),
  ('Wells Fargo Loan (via Dad)',         'loan',        21420.00, 12.49,  1100.00, 'Roll into HELOC — 12.49% vs 6.85%',        NULL),
  ('Virginia FCU Loan (via Dad)',        'loan',        18600.00,  9.25,   350.00, 'Roll into HELOC — 9.25% vs 6.85%',         NULL),
  ('AmEx Platinum — Old Plan It®',      'credit_card',  2252.41, 15.50,   230.97, 'Roll into HELOC — 15.5% vs 6.85%',         NULL),
  ('AmEx Platinum — New Plan It®',      'credit_card',  2835.58, 15.50,   270.12, 'Roll into HELOC — 15.5% vs 6.85%',         NULL),
  ('AmEx Platinum — Revolving',         'credit_card',   513.11, 27.49,     NULL, 'Roll into HELOC — 27.49% revolving',        'Revolving balance'),
  ('AmEx Gold — Plan It®',              'credit_card',  1776.88, 15.50,   182.18, 'Roll into HELOC — 15.5% vs 6.85%',         NULL),
  ('AmEx Personal Loan',                'loan',         3942.57,  7.33,   407.01, 'Keep — 7.33% close to HELOC rate',          NULL),
  ('Nordstrom (Synchrony)',             'credit_card',   653.43, 29.40,    40.00, 'Roll into HELOC — 29.4% revolving',         NULL),
  ('Citi Best Buy 4802',                'credit_card',  2230.40,  0.00,    50.00, 'Promo 0% — pay from HELOC before expiry',   '0% promo'),
  ('Chase Prime Visa',                  'credit_card',   627.73,  0.00,    95.77, 'Promo 0% — pay from cash/HELOC',            '0% promo'),
  ('PayPal Credit',                     'credit_card',   124.35,  0.00,    30.00, 'Promo 0% — small, pay from cash',           'Deferred interest'),
  ('BofA Mortgage',                     'mortgage',   498903.37,  3.375, 2486.79, 'Keep — 3.375% fixed, do not touch',         '2214 N Carroll Ave Dallas TX');

-- ── 2. financial_modules ─────────────────────────────────────

TRUNCATE TABLE financial_modules RESTART IDENTITY CASCADE;

INSERT INTO financial_modules (module_number, name, category, status, progress, description, details) VALUES
  (1, 'Spending Analysis', 'Analysis', 'in_progress', 90,
  ...
  (2, 'Subscription Audit', 'Analysis', 'in_progress', 90,
  ...
  (4, 'Debt Inventory & HELOC', 'Debt', 'in_progress', 70,
   ...
   '{"docsHave":["All credit card statements (Feb 2026)","SoFi loan details","LightStream amortization","HELOC offer — Texas CU $190K at 6.85%"],"docsMissing":["Chase Freedom card statements (promo confirmation)","Virginia FCU loan statement","Wells Fargo loan statement"],"actions":["Confirm HELOC closing date with Loan Depot","Verify Chase Freedom cards are 100% at 0%","Get Virginia FCU + Wells Fargo current balance statements"]}'
  ),
  (5, '401(k) Review', 'Retirement', 'not_started', 0,
  ...
  (6, 'Property Tax Protest (DCAD)', 'Tax', 'not_started', 0,
  ...
  (7, 'IRS Balance Resolution', 'Tax', 'not_started', 0,
  ...
  (8, 'Savings & Wealth Plan', 'Wealth', 'not_started', 0,
...
INSERT INTO subscriptions (name, amount, billing_cycle, category, action, notes, is_active) VALUES
  ...
  ('LifeTime Fitness',          228.23, 'monthly', 'Health',     'review', '⚠️ BIGGEST WIN — Consider LA Fitness at $25/mo',            true),
